...
class UsbTransport implements DeviceTransport {
  UsbTransport({required UsbWorker worker, required this.kind})
    : _worker = worker {
  ...
  Future<List<DevicePort>> scan() async {
    ...
    return rows.map((row) {
      ...
      return DevicePort(item[0] as String, item[1] as String, kind);
    }).toList();
  }

  @override
  Future<void> connect(DevicePort port) async {
    if (_disposed) throw StateError('$kind 已关闭');
    if (port.kind != kind) throw ArgumentError('不是 $kind 端口');
    await disconnect();
    final epoch = _epoch;
    if (_disposed) throw StateError('$kind 已关闭');
    _hasSession = true;
    ...
    if (_disposed || epoch != _epoch) throw StateError('$kind 连接已取消');
    _online = true;
    _connected.add(true);
  }
  ...
  Future<void> send(Uint8List frame) async {
    if (!_online || _disposed) throw StateError('$kind 未连接');
    if (frame.isEmpty || frame.length > payload) {
      throw ArgumentError('$kind 帧长度必须为 1～244 字节');
    }
