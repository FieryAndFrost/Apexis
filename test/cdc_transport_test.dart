...
import 'package:apexis/transport/cdc_device_filter.dart';
...
void main() {
  test(
    'CDC whitelist only selects GT1 trial ACM interface, not DEBUG/MIDI',
    () {
      expect(isGt1CdcHardwareId(r'USB\VID_3654&PID_4B55&MI_03'), isTrue);
      expect(
        isGt1CdcHardwareId(r'USB\VID_3654&PID_4B55&REV_0100&MI_03'),
        isTrue,
      );
      for (final id in [
        r'USB\VID_3654&PID_4D55&MI_03',
        r'USB\VID_3654&PID_4B55&MI_00',
        r'USB\VID_1209&PID_0001&MI_03',
        r'USB\VID_3654&PID_4B55&MI_030',
      ]) {
        expect(isGt1CdcHardwareId(id), isFalse);
      }
      expect(cdcPortFromName('USB Serial Device (COM12)'), 'COM12');
      expect(cdcPortFromName('DEBUG'), isNull);
    },
  );
  test('CDC scan, copied TX, byte stream, disconnect and reconnect', () async {
    final worker = CdcWorker(fakeCdc);
    final transport = WindowsCdcTransport(worker: worker);
    final states = <bool>[];
  ...
  test('CDC rejects oversized frames and wrong port types', () async {
    final transport = WindowsCdcTransport(worker: CdcWorker(fakeCdc));
    await expectLater(
  ...
  test('CDC unplug reports fault and rejects further writes', () async {
    final worker = CdcWorker(fakeCdc);
    final transport = WindowsCdcTransport(worker: worker);
    final errors = <Object>[];
  ...
  test('cancel during CDC open closes the newly opened handle', () async {
    final worker = CdcWorker(fakeCdc);
    final transport = WindowsCdcTransport(worker: worker);
    final port = (await transport.scan()).single;
  ...
  test('CDC worker deadline retires session without hanging caller', () async {
    final worker = CdcWorker(
      fakeCdc,
  ...
  test('CDC worker unexpected exit completes request with error', () async {
    final worker = CdcWorker(fakeCdc);
    await worker.request('status');
