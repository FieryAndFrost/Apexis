...
import 'package:apexis/transport/macos_usb_transport.dart';
...
void main() {
  for (final kind in ['USB WinUSB', 'USB IOKit']) {
    DeviceTransport create({required UsbWorker worker}) => kind == 'USB WinUSB'
        ? WindowsUsbTransport(worker: worker)
        : MacosUsbTransport(worker: worker);
    group(kind, () {
      test('WinUSB early RX failure never publishes connected=true', () async {
        final worker = UsbWorker(fakeUsb);
        final transport = create(worker: worker);
        final states = <bool>[];
        final errors = <Object>[];
        final stateSub = transport.connected.listen(states.add);
        final byteSub = transport.bytes.listen((_) {}, onError: errors.add);
        await expectLater(
          transport.connect(DevicePort('fault-during-open', 'GT1', kind)),
          throwsStateError,
        );
      ...
      test('WinUSB rejects oversized frames and wrong port types', () async {
        final transport = create(worker: UsbWorker(fakeUsb));
        await expectLater(
      ...
      test('WinUSB unplug reports fault and rejects further writes', () async {
        ...
        await transport.connect((await transport.scan()).single);
        final disconnected = transport.connected.firstWhere(
          (online) => !online,
        );
      ...
      test(
        'cancel during WinUSB open closes the newly opened handle',
        () async {
          final worker = UsbWorker(fakeUsb);
          final transport = create(worker: worker);
          final port = (await transport.scan()).single;
          final entering = worker.events.firstWhere(
            (event) => event[0] == 'opening',
          );
          final opening = expectLater(
      ...
      test(
        'WinUSB worker unexpected exit completes request with error',
        () async {
          final worker = UsbWorker(fakeUsb);
          await worker.request('status');
