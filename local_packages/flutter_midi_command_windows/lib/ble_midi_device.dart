// BLE MIDI 协议状态名沿用上游实现，避免 fork 与平台包产生无关差异。
...
class BLEMidiDevice extends MidiDevice {
  ...
  BLEMidiDevice(this.deviceId, String name, this._rxStreamCtrl)
      : super(deviceId, name, 'BLE', false);
  ...
  disconnect() {
    UniversalBle.unsubscribe(
        deviceId, _midiService!.uuid, _midiCharacteristic!.uuid);
    ...
    if (bytes.first == 0xF0 && bytes.last == 0xF7) {
      ...
      if (bytes.length > packetSize - 3) {
        ...
        while (dataBytes.isNotEmpty) {
          ...
          packet = dataBytes
              .getRange(0, pickCount)
              .toList(); // Pick bytes for packet
          // Insert header
          packet.insert(0, 0x80);
  ...
  _discoverServices() async {
    ...
    _midiService = services
        .where((service) => service.uuid.toUpperCase() == MIDI_SERVICE_ID)
        .firstOrNull;
      ...
      for (int i = 1; i < packet.length; i++) {
        ...
        if ((((midiByte & 0x80) == 0x80) &&
          ...
          switch (bleHandlerState) {
            ...
            case BLE_HANDLER_STATE
                  .PARAMS: // After params can come TSlow or more params
              break;
            ...
            default:
              debugPrint('Unhandled state $bleHandlerState');
              break;
          }
        }
        ...
        switch (bleHandlerState) {
          ...
          case BLE_HANDLER_STATE.PARAMS:
            ...
            if (bleMidiPacketLength == bleMidiBuffer.length) {
              _createMessageEvent(bleMidiBuffer, timestamp);
              bleMidiBuffer.removeRange(
                  1,
                  bleMidiBuffer
                      .length); // Remove all but status, which might be used for running msgs
            }
            break;
          ...
          default:
            debugPrint('Unhandled state (data) $bleHandlerState)');
            break;
        }
      }
    }
