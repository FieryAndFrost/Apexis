...
/// Only messages cross this boundary; all WinMM handles/buffers stay in the
/// worker. A synchronous driver call must never run on the Flutter isolate.
class MidiWorker {
  MidiWorker(this.entryPoint, {this.timeout = const Duration(seconds: 5)});
  ...
  Future<void> _start() => _starting ??= (() async {
        ...
        _errors = ReceivePort()
          ..listen((dynamic error) {
            _fail(StateError('Windows MIDI 工作线程异常：$error'));
          });
        _exits = ReceivePort()
          ..listen((_) {
            if (!_disposed && _failure == null) {
              _fail(StateError('Windows MIDI 工作线程已退出，请重新启动应用'));
            }
            _receive?.close();
            _errors?.close();
            _exits?.close();
        ...
        try {
          await Future.wait<Object?>([
            Isolate.spawn(entryPoint, _receive!.sendPort,
                onError: _errors!.sendPort,
                onExit: _exits!.sendPort,
                errorsAreFatal: true,
                debugName: 'Windows MIDI'),
            waiting,
          ], eagerError: true);
        } catch (e) {
          _fail(StateError('Windows MIDI 工作线程启动失败：$e'));
          rethrow;
        }
      })();
  ...
  Future<Object?> request(String operation, [Object? arguments]) {
    ...
    _tail = _tail.then((_) async {
      try {
        if (_disposed) throw StateError('Windows MIDI 已关闭');
        ...
        try {
          ...
          final error = StateError('Windows MIDI $operation 超时，已停止本次连接；界面仍可操作。'
              '请关闭并重新启动应用，若仍无法连接再检查 Windows MIDI 服务。');
          _fail(error);
  ...
  Future<void> dispose() async {
    if (_disposed) return;
    _disposed = true;
    _fail(StateError('Windows MIDI 已关闭'));
    _commands?.send(['shutdown']);
    await _events.close();
  }
}
